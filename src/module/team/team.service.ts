...
class TeamService implements TeamServiceInterface {
  ...
  > => {
    const team = await this.teamRepo.getTeamByUserId(userId);
    const week = await this.weekService.getWeek("current");
  ...
  getTeamRecordes = async (weekId: number, teamId: number) => {
    return this.replacementLogRepo.getTeamRecordes(weekId, teamId);
  };

  getFeildTeamById = async (id: number): Promise<TeamOutputType | null> => {
    return this.teamRepo.getFeildTeamById(id);
  };

  getFeildTeamByUserId = async (
    userId: number
  ): Promise<TeamOutputType | null> => {
    return this.teamRepo.getFeildTeamById(userId);
  };
}
